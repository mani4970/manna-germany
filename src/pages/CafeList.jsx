    ...
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <div style={{ position: 'relative', width: '100%', height: '220px', background: C.surface2 }}>
          {photos.length > 0 ? (
            <img src={photos[photoIdx]} alt={place.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          ) : (
        ...
        <div style={{ padding: '20px 20px 0' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            {place.priceLevel && <span style={{ fontSize: '14px', color: C.textSub, flexShrink: 0 }}>{priceLabel(place.priceLevel)}</span>}
          </div>
          {place.rating && (
            <p style={{ color: C.goldDim, fontSize: '13px', marginTop: '4px' }}>⭐ {place.rating} ({place.userRatingsTotal?.toLocaleString()})</p>
          )}
          {place.distanceMeters && (
            <p style={{ color: C.textSub, fontSize: '12px', marginTop: '4px' }}>{place.distanceMeters}m · {Math.round(place.distanceMeters / 80)} min {L.walk}</p>
          )}
          {tags.length > 0 && (
            <div style={{ display: 'flex', gap: '6px', marginTop: '10px', flexWrap: 'wrap' }}>
              {tags.map((t, i) => (
                <span key={i} style={{ padding: '3px 10px', borderRadius: '20px', fontSize: '11px', background: t.color + '30', color: t.color }}>{t.label}</span>
              ))}
            </div>
          )}

...
export default function OccasionSelect({ city, spot, lang, L, onNext, onBack }) {
  const emojis = ['💑', '👥', '👨‍👩‍👧‍👦', '💼']
    ...
    <div style={{ padding: '24px', background: C.bg, minHeight: '100vh', fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div style={{ paddingTop: '20px', marginBottom: '28px', textAlign: 'center' }}>
        <div style={{ display: 'flex', gap: '6px', marginBottom: '28px' }}>
          {[1,2,3,4,5,6].map(i => (
            <div key={i} style={{ flex: 1, height: '2px', borderRadius: '2px', background: i <= 3 ? C.gold : C.border }} />
          ))}
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        {L.occasions.map((occ, i) => (
          <button key={i} onClick={() => onNext(occ)} className="no-orange-card"
            style={{
          ...
          >
            ...
            <div style={{ fontWeight: '400', fontSize: '14px', color: C.text }}>{occ}</div>
            <div style={{ fontSize: '11px', color: C.textSub, fontWeight: '300' }}>{L.occasion_descs[i]}</div>
          </button>
        ))}
      </div>
    </div>

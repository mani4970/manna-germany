...
function PriceLevel({ level }) {
  ...
  return (
    <span style={{ fontSize: '12px' }}>
      <span style={{ color: C.gold, fontWeight: '600' }}>{won}</span>
      <span style={{ color: C.border }}>{gray}</span>
    </span>
  )
}
...
export default function RestaurantList({ selections, onNext, onBack, referencePoint }) {
  ...
  useEffect(() => {
    if (!basePoint?.lat || !basePoint?.lng) { setError('위치 정보가 없습니다'); setLoading(false); return }
    setLoading(true); setError(null)
    const cuisineParam = selections.restaurantCuisine && selections.restaurantCuisine !== 'all' ? '&cuisine=' + selections.restaurantCuisine : ''
    fetch('/api/places/search?type=restaurant&lat=' + basePoint.lat + '&lng=' + basePoint.lng + '&radius=1000' + cuisineParam)
      .then(r => r.json())
      .then(data => {
      ...
      <div style={{ padding: '16px 24px 0' }}>
        {loading && (
          <div style={{ textAlign: 'center', padding: '60px 0', color: C.textSub }}>
            ...
            <p style={{ fontWeight: '300' }}>근처 레스토랑 찾는 중...</p>
          </div>
        )}
        {error && (
          <div style={{ textAlign: 'center', padding: '60px 0', color: C.textSub }}>
        ...
        {!loading && !error && places.length === 0 && (
          <div style={{ textAlign: 'center', padding: '60px 0', color: C.textSub }}>
            <div style={{ fontSize: '32px', marginBottom: '12px' }}>🤔</div>
            <p style={{ fontWeight: '300' }}>근처에 레스토랑이 없어요</p>
          </div>
        )}
